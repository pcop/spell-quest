...
- `tools/generate-phonics-audio.sh` — legacy espeak-ng-based phonics generator, superseded by `generate-neural-audio.py`. **Do not use for new chunks** — it produces robotic espeak-ng audio that audibly clashes with the neural-voice clips the rest of `phonics-audio/` now uses. Kept only for its debugging notes on espeak-ng phoneme codes.
...
- `phonics` (required): natural-phonics letter-group breakdown, e.g. `{ "chunks": ["c","a","t"], "silent": [] }`. `chunks` concatenated must equal `word`; `silent` is a list of 0-indexed positions into `chunks` that are silent (e.g. silent e). Chunking rules: consonant digraphs (ch/sh/th/ck/wh/nk), vowel teams (ee/ea/oo/ow/ou/ay/ue/eigh/oa/aw), r-controlled vowels (ar/er/ir/or/ur/air/ear/our/oor), and doubled letters (ll/rr/pp) are each one chunk; consonant blends (br/pl/st, etc.) stay split into individual letters since they're two fast sounds, not one. A chunk's audio is looked up purely by its text, so identical spellings always get the same pronunciation even when the real pronunciation varies by word (documented known limitation — see `oo`/`ear` in README's 已知待驗證項目).
  - `audioOverrides` (optional): `{ "<chunkIndex>": "<virtualChunkId>" }`, for the rare case where one specific word's pronunciation of a chunk genuinely conflicts with every other word sharing that same chunk spelling — e.g. `fly`'s word-final `y` is long-i (/aɪ/), but the shared `y` chunk audio is long-e (/i/, from `cherry`/`happy`-style words) since that's the more common case. The override only swaps which `phonics-audio/<file>.mp3` gets played for that one chunk index; `chunks` itself (display text, concatenation-equals-`word` invariant, fallback-TTS text) is untouched. The override target is a "virtual chunk id" registered in `CHUNK_CONFIG` in `tools/generate-neural-audio.py` (e.g. `y-long-i`, sourced from `sky`) — it deliberately isn't a chunk spelling that appears in any `phonics.chunks` array, so it can't collide with the by-text lookup. See `phonicsChunkAudioName()` in `app.js`.
- **If a new word needs a phonics chunk that isn't among the current set**, add an entry to the `CHUNK_CONFIG` table in `tools/generate-neural-audio.py` (a carrier word plus `start`/`end` trim seconds isolating the target sound) and re-run it to generate the new `phonics-audio/<chunk>.mp3`. `phonics.chunks` being valid data does *not* mean the audio exists — that's a separate generation step. Do not use the legacy `tools/generate-phonics-audio.sh` for this — it generates espeak-ng audio, not the neural-voice audio the rest of `phonics-audio/` uses.
...
**Phonics & Words audio pipeline**: Uses pre-generated Microsoft Edge Neural Voice (`en-US-JennyNeural`) audio for all word pronunciations (`words-audio/*.mp3`) and 53 phonics chunks (`phonics-audio/*.mp3`), both generated by `tools/generate-neural-audio.py` (see "File structure" above — the older `tools/generate-phonics-audio.sh` is legacy and no longer the source of the shipped `phonics-audio/` files). Both whole-word (`speakWord()`) and chunk breakdown (`speakPhonics()`) use `Audio` elements with dynamic preloading, `playbackRate` speed adjustment, and fallback to browser `SpeechSynthesis` if an audio asset fails to load.
